...
16. **`StyledInputBase<TValue>` for real `EditContext`/validation-backed form controls; `StyledComponentBase` for everything else.** A component needs `EditContext`/`ValueExpression`/`FieldIdentifier` support (validation state, `EditForm` integration, true drop-in parity with a framework `Input*` control) only when it must derive from `Microsoft.AspNetCore.Components.Forms.InputBase<TValue>` — C# has no multiple inheritance, so it cannot also extend `StyledComponentBase`. `StyledInputBase<TValue>` (`src/CanDoItAll.Components.BaseLib/Components/Forms/StyledInputBase.cs`) replicates `StyledComponentBase`'s `Class`/`Style`/`BuildAttributes` contract by composition instead, and folds `EditContext.FieldCssClass(FieldIdentifier)` into the merged class string — deliberately bypassing `InputBase<TValue>.CssClass` (which independently reads `AdditionalAttributes["class"]` + `FieldCssClass`) to avoid double-emitting the consumer's `Class`. Default to `StyledComponentBase` for any component using plain `Value`/`ValueChanged` two-way binding without `EditForm` involvement (the existing `TextBox`/`Numeric<TValue>`/`TextArea`/`DropDown<TValue>`/`CheckBox` family) — `StyledInputBase<TValue>` is strictly for components whose entire purpose is `EditForm` parity, since it forces every call site into an `EditForm`/cascading-`EditContext` ancestor (`InputBase<TValue>` throws `InvalidOperationException` otherwise). `TextInput`/`NumberInput<TValue>`/`DateInput<TValue>`/`TextAreaInput`/`SelectInput<TValue>` are the reference implementations.

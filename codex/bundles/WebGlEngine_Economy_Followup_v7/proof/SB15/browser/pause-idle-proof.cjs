...
(async () => {
  ...
  await page.waitForFunction(() => {
    ...
    const status = document.querySelector('[data-testid="webgl-run-status"]')?.textContent?.trim() || "";
    return playing === "True" || status.startsWith("Playing");
  }, null, { timeout: 5000 });
  const injectedMotion = await enqueueLongMotion(page);
  const activeRuntimeBeforePauseHandle = await page.waitForFunction(() => {
    const host = document.querySelector('[data-testid="webgl-scene-host"]');
    const diagnostics = host && window.CanDoItAll?.webglScene?.getDiagnostics(host);
    return (diagnostics?.activeMotionCount || 0) > 0 ? diagnostics : false;
  }, null, { timeout: 5000 });
  const activeRuntimeBeforePause = await activeRuntimeBeforePauseHandle.jsonValue();

  const beforePause = {
    status: await status(page),
    activeRuntimeDiagnostics: activeRuntimeBeforePause,
  ...
  await page.waitForFunction(() => {
    ...
    return playing === "False" &&
      ...
      (diagnostics?.runtimeStopGeneration || 0) > 0 &&
      diagnostics?.lastRuntimeStopIdle === true &&
      diagnostics?.lastRuntimeStopTimedOut === false;
  }, null, { timeout: 6000 });
  ...
  const artifact = {
    ...
    assertions: {
      ...
      activeMotionWasPresentBeforePause: (beforePause.activeRuntimeDiagnostics?.activeMotionCount || 0) > 0,
      pauseSetsPlayingFalse: afterPause.status.playing === "False",
      pauseStatusSettled: afterPause.status.status === "Paused.",
      afterLateDrainStillPaused: afterLateDrain.status.playing === "False" && afterLateDrain.status.status === "Paused.",

...
const browserDir = path.join(repoRoot, "codex", "bundles", "WebGlEngine_Economy_Followup_v13", "proof", "SB06", "browser");
const route = "http://127.0.0.1:5327/run-playback";
const screenshotPath = path.join(browserDir, "runtime-idle-pause-after.png");
const assertionsPath = path.join(browserDir, "runtime-idle-pause-assertions.json");
const consolePath = path.join(browserDir, "runtime-idle-pause-console.log");
...
try {
  ...
  const disallowedConsole = consoleMessages.filter(item =>
    item.type === "error" ||
    item.type === "pageerror");
  const browserRuntimeStopGeneration = Number(after500Sample.diagnostics?.runtimeStopGeneration || 0);
  const uiRuntimeStopGeneration = Number(uiDiagnostics.runtimeStopGeneration || 0);
  const assertions = {
    route,
    viewport: "1920x1080",
    beforePause,
    ...
    settledElapsedMs,
    settledSample,
    browserRuntimeStopGenerationAdvanced: browserRuntimeStopGeneration >= 1,
    uiRuntimeStopGenerationSynced: uiRuntimeStopGeneration >= 1,
    after500: {
      ...after500Sample,
  ...
  if (!assertions.browserRuntimeStopGenerationAdvanced) {
    throw new Error("Browser runtime diagnostics did not expose advanced runtimeStopGeneration after pause.");
  }

  if (!assertions.uiRuntimeStopGenerationSynced) {

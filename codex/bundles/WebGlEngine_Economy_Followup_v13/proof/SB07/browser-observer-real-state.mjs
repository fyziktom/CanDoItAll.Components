...
const proofDir = path.join(
    ...
    "codex",
    "bundles",
    "WebGlEngine_Economy_Followup_v13",
    "proof",
    "SB07");
const route = process.env.CDA_SB07_WEBGL_URL || "http://127.0.0.1:5327/run-playback";
...
try {
    ...
    await page.evaluate(() => {
        window.__sb07RunPlaybackError = "";
        window.__sb07RunPlaybackPromise = window.CanDoItAll.webglSandbox.runPlayback
            .play()
            .catch(error => {
                window.__sb07RunPlaybackError = error?.message || String(error);
            });
        return true;
    });
    ...
    const playBridgeError = await page.evaluate(() => window.__sb07RunPlaybackError || "");
    ...
    for (const [name, passed] of Object.entries(assertions)) {
        if (typeof passed === "boolean" && !passed) {
            throw new Error(`SB07 browser observer assertion failed: ${name}`);
        }
    }
    ...
    console.log(`SB07 browser observer proof passed: ${observerProofPath}`);
} finally {
    await browser.close();
}

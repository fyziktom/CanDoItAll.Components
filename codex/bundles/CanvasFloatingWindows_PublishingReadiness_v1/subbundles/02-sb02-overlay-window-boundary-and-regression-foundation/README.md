...
- Browser proof for `/groups/overlays` window open states.
...
5. Use browser proof on `/groups/overlays` to open, minimize, restore, reset, hide/show, drag, and resize the window.
...
- Playwright/MCP transcript and screenshots for `/groups/overlays`.
...
- Route: `/groups/overlays?scenario=happy-path`, plus dense and long-text when available.
...
```text
Execute SB02 only. Prove OverlayLib owns generic floating-window behavior with tests, source assertions, and /groups/overlays browser proof. Preserve public behavior and stop if Canvas-specific changes become necessary.
```

# SB05 — Async session persistence and snapshot store boundaries
...
Remove sync-over-async snapshot store usage and define async session persistence boundaries.
...
- Add async export/import APIs or split sync and async store contracts clearly.
- Avoid `.AsTask().GetAwaiter().GetResult()` in server/UI path.
- Add cancellation-token-aware persistence where practical.
- Document which operations are safe to call from Blazor event handlers.
...
1. Add async methods to service interface with compatibility sync wrappers only when no async store is used.
2. Update UI to await async export/import/snapshot-store paths where used.
3. Add scan preventing sync-over-async in SimulationSandbox.
...
- Focused tests with fake delayed snapshot store.
- Static scan: no `.GetAwaiter().GetResult()` in SimulationSandbox production files.
- Import/export hash validation still passes.
...
The subbundle can close only when all acceptance checklist items are complete, `proof/SB05/manifest.md` is updated, semantic invariants are updated for critical subbundles, and downstream dependency impacts are reviewed.

## Suggested Agent Prompt

You are implementing `SB05` from the CanDoItAll WebGL/Economy v3 follow-up hardening bundle. Read this README, the phase plan, traceability, and source references. Work only inside this subbundle scope. Produce failing-first and passing proof, update manifests, and stop at the progression gate before starting downstream work.

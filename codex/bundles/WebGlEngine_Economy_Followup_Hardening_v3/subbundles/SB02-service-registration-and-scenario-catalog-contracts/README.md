# SB02 — Reusable service registration and scenario catalog contracts
...
Make Economy simulation sandbox services reusable outside Node.
...
- Add service-registration extension(s), e.g. AddEconomySimulationSandbox or AddEconomySimulationSandboxComponents.
- Provide default in-memory/file-system catalog registration hooks.
- Keep Node-specific root selection in Node but avoid copy-paste DI for consumers.
- Document how external apps register scenario catalogs.
...
1. Design extension signatures in SimulationSandbox or Components without pulling UI into headless packages.
2. Update Node to use the extension.
3. Update tests to use the extension instead of manual per-test wiring.
4. Add a non-Node sample/host test that renders the page through reusable registration.
...
- BUnit test using only public extension registration.
- Build Node and an external/demo host.
- Scan proving only Node owns Node path configuration.
...
The subbundle can close only when all acceptance checklist items are complete, `proof/SB02/manifest.md` is updated, semantic invariants are updated for critical subbundles, and downstream dependency impacts are reviewed.

## Suggested Agent Prompt

You are implementing `SB02` from the CanDoItAll WebGL/Economy v3 follow-up hardening bundle. Read this README, the phase plan, traceability, and source references. Work only inside this subbundle scope. Produce failing-first and passing proof, update manifests, and stop at the progression gate before starting downstream work.

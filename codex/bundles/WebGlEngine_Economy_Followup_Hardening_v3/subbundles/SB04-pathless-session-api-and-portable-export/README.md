# SB04 — Pathless session API and portable export/import
...
Add pathless session APIs and portable export/import semantics.
...
- Add LoadByScenarioId or Load(EconomySimulationScenarioPack) APIs.
- Keep path-based APIs as compatibility wrappers.
- Session export should preserve scenario id + pack hash, not rely on absolute paths.
- Import should resolve through a catalog where possible.
...
1. Introduce scenario reference/session source DTO.
2. Implement catalog-backed load/export/import.
3. Add migration-compatible import handling for old path-based exports.
4. Update UI and tests to use scenario-id flow.
...
- Move scenario directory after export; import still works through catalog.
- Negative test: same scenario id with different pack hash is rejected.
- No runtime UI path points to tests directory.
...
The subbundle can close only when all acceptance checklist items are complete, `proof/SB04/manifest.md` is updated, semantic invariants are updated for critical subbundles, and downstream dependency impacts are reviewed.

## Suggested Agent Prompt

You are implementing `SB04` from the CanDoItAll WebGL/Economy v3 follow-up hardening bundle. Read this README, the phase plan, traceability, and source references. Work only inside this subbundle scope. Produce failing-first and passing proof, update manifests, and stop at the progression gate before starting downstream work.

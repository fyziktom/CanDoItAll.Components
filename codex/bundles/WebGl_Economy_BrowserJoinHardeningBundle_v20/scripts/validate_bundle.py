...
SUBBUNDLE_IDS = [f"SB{index:02d}" for index in range(1, 15)]
CRITICAL_SUBBUNDLES = {"SB01", "SB02", "SB03", "SB04", "SB05", "SB06", "SB07", "SB11", "SB14"}
...
REQUIRED_PREPARED = [
    ...
    "00_context/02_test_results_observed.md",
    "00_context/03_real_test_status.md",
    "01_architecture/01_next_join_pipeline.md",
    "01_architecture/02_potential_examples_generic_mapping.md",
    "01_architecture/03_browser_smoke_scope.md",
    "04_validation/validation_commands.md",
    "04_validation/forbidden_reference_policy.md",
    "07_references/source_references.md",
    "08_probe_scenarios/shared_resource_probe.md",
    "08_probe_scenarios/finite_resource_probe.md",
    "09_performance/performance_risk_register.md",
    "10_workflow/workflow_alignment.md",
]

SUBBUNDLE_REQUIRED_SECTIONS = [
    "## Status",
    "## Goal",
    "## Prerequisites",
    "## Owned Requirements",
...
def check_prepared(errors: list[str]) -> None:
    ...
    requirements = read(ROOT / "requirements/01-normalized-requirements.md")
    for marker in ["## Critical Path Risks", "## Validation Risks", "## Reopen Triggers"]:
        if marker not in requirements:
            errors.append(f"requirements/01-normalized-requirements.md missing {marker}")
    ...
    for subbundle_id in SUBBUNDLE_IDS:
        ...
        text = read(subbundle)
        for section in SUBBUNDLE_REQUIRED_SECTIONS:
            if section not in text:
                errors.append(f"{relative(subbundle)} missing {section}")

        if subbundle_id in CRITICAL_SUBBUNDLES:
            for marker in ["Semantic Adequacy Gate", "Shallow-pass trap", "Adversarial negative proof", "Semantic positive proof", "Anti-stub audit"]:
                if marker not in text:
                    errors.append(f"{relative(subbundle)} critical proof contract missing {marker}")
...
def check_completed(errors: list[str]) -> None:
    report = read(ROOT / "reviews/01-execution-report.md")
    pending_markers = ["| Pending |", "| Prepared |", "| In progress |", "| Not started |"]
    ...
    final_red_team = ROOT / "proof" / "SB14" / "final-fake-proof-resistance.md"
...
def scan_forbidden_instruction_text(errors: list[str]) -> None:
    for path in ROOT.rglob("*.md"):
        ...
        for term in ["small-screen optimization", "mobile optimization", "tablet optimization"]:
            ...
            if term in text and not any(marker in context for marker in ["do not", "large-screen", "desktop", "forbidden", "out of scope", "must not", "no mobile"]):

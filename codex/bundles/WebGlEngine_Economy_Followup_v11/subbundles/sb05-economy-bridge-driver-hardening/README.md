# SB05 — Economy bridge driver hardening
...
Make Economy WebGL bridge depend on the driver contract, verify generic IDs are stable hashes, and prove no economy terms leak to WebGlRunDocument except source provenance.
...
- `proof/SB05/manifest.md` updated with artifact paths, commands, and pass/fail status.

## Semantic invariants

- Economy terms may exist in Economy bridge inputs and private diagnostics, but generic WebGL run documents must be domain-neutral.
...
- `SB05_FocusedContractTests`
- `SB05_BoundaryAuditTests`
- `SB05_ProofArtifactValidationTests`

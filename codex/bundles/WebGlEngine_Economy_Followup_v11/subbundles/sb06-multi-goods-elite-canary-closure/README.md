# SB06 — Multi-goods-elite canary closure
...
Turn multi-goods-elite from a scenario file into a full research canary: all events handled, strict diagnostics clean, explicit store policies, metrics/invariants, manifest, and CLI/headless proof.
...
- `proof/SB06/manifest.md` updated with artifact paths, commands, and pass/fail status.

## Semantic invariants

- Canary must fail loudly if exchange/investment semantics are only partially represented.
...
- `SB06_FocusedContractTests`
- `SB06_BoundaryAuditTests`
- `SB06_ProofArtifactValidationTests`

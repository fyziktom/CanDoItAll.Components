(function () {
    ...
    function resolveContainer(host, options) {
        const selector = options?.containerSelector;
        if (selector) {
            const resolved = host.closest(selector);
            if (resolved) {
                return resolved;
            }
        }

        return host.closest("[data-cda-overlay-container], .cw-stage-surface, .wgl-workbench-stage")
            || host.parentElement;
    }

    function resolveSafeTopElement(container, options) {
        const selector = options?.safeTopSelector;
        if (selector) {
            if (typeof container.matches === "function" && container.matches(selector)) {
                return container;
            }

            const scopedMatch = container.querySelector(selector);
            if (scopedMatch) {
                return scopedMatch;
            }
        }

        const frame = container.closest("[data-cda-overlay-frame], .cw-workbench-frame, .wgl-workbench-shell");
        if (frame) {
            const frameMatch = frame.querySelector("[data-cda-overlay-safe-top], .cw-toolbar, .wgl-overlay-toolbar, .wgl-workbench-toolbar");
            if (frameMatch) {
                return frameMatch;
            }
        }

        return container.querySelector("[data-cda-overlay-safe-top]");
    }

    function resolveSafeTop(container, options) {
        if (!container) {
            return 16;
        }

        const margin = resolveMargin(container);
        const safeTopElement = resolveSafeTopElement(container, options);
        if (!safeTopElement) {
            return margin;
        }

        const containerRect = container.getBoundingClientRect();
        const safeTopRect = safeTopElement.getBoundingClientRect();
        return Math.max(margin, Math.round(safeTopRect.bottom - containerRect.top + 12));
    }
    ...
    function resolveMaxHeight(container, options) {
        const margin = resolveMargin(container);
        const safeTop = resolveSafeTop(container, options);
    ...
    function hasCustomGeometry(windowState) {
        return !!(windowState &&
            (windowState.left > 0 ||
                windowState.top > 0 ||
                windowState.width > 0 ||
    ...
    function applyWindowGeometry(state) {
        ...
        host.style.resize = minimized || options.allowResize === false ? "none" : "both";
    ...
    function scheduleLayoutRefresh(state) {
        if (hasCustomGeometry(state.options.state)) {
            return;
        }

        window.requestAnimationFrame(() => {
            window.requestAnimationFrame(() => {
                if (!state.host.isConnected || !state.container || hasCustomGeometry(state.options.state)) {
                    return;
                }

                applyWindowGeometry(state);
                scheduleGeometryCommit(state, 0);
            });
        });
    }
    ...
    function clampDraggedWindow(state, nextLeft, nextTop) {
        ...
        const safeTop = resolveSafeTop(state.container, state.options);
    ...
    function syncWindowHandle(state) {
        const handle = state.host.querySelector("[data-cda-overlay-drag], [data-cw-window-drag]");
    ...
    function syncWindowObservers(state) {
        ...
        state.resizeObserver = createResizeObserver(() => {
            if (!state.host.isConnected || !state.container || state.host.classList.contains("is-dragging")) {
                return;
            }
    ...
    function createWindowState(host, dotNetRef, options) {
        const state = {
            ...
            dotNetRef,
            options,
            container: resolveContainer(host, options),
            handle: null,
            resizeObserver: null,
            geometryTimer: 0,
    ...
    function updateWindowState(host, dotNetRef, options) {
        const normalized = {
            windowId: options.windowId || host.dataset.windowId || "overlay-window",
            placement: options.placement || "top-right",
            containerSelector: options.containerSelector || "",
            safeTopSelector: options.safeTopSelector || "",
        ...
        state.container = resolveContainer(host, normalized);
    ...
    root.overlayWindow = overlayWindowApi;
    root.canvasFloatingWindow = overlayWindowApi;
})();
